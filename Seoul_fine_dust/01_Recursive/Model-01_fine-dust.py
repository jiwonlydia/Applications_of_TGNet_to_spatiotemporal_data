...
def single_step_forecast(STAMP, LAG):
    ...
    if not os.path.exists('./output'):
        os.mkdir('./output')
    save_test_output(y_pred_inv, y_true_inv, output_path='./output/stamp{STAMP}_lag{LAG}_step1')
...
step = int(24/args.stamp)
if args.train == False:
    recursive_forecast(STAMP=args.stamp, LAG=args.lag, STEP=step)
else: 
...
# rmse_table.to_csv('rmse_table_recursive.csv')
